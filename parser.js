...
function normalizeText(t) {
  ...
  return t.replace(/\u00A0/g, ' ')
          ...
          .replace(/[\t ]+/g, ' ')
          .replace(/\r/g, '\n')
          // Убираем пробелы внутри дат: "12.01 .1990" -> "12.01.1990", ".12.01 .1990" -> "12.01.1990"
          .replace(/(\d{1,2})\s*\.\s*(\d{1,2})\s*\.\s*(\d{4})/g, '$1.$2.$3')
          // Убираем ведущую точку перед датой: ".12.01.1990" -> "12.01.1990"
          .replace(/(?<!\d)\.(\d{1,2}\.\d{1,2}\.\d{4})/g, '$1')
          .trim();
}
...
function extractBorrowers(text, contractDate = null) {
  ...
  for (const line of lines) {
    ...
    const dateBeforeGenderPattern = /(\d{1,2}\.\d{1,2}\.\d{4})\s*(?:г\.?\s*)?(ЖЕНЩИНА|МУЖЧИНА|ЖЕН|МУЖ|ОНА|ОН|МУЖЧ|женщина|мужчина|жен|муж|она|он|мужч)/ig;
...
function parseTextToObject(rawText) {
  ...
  if (/(таунхаус|таун)/i.test(text)) result.objectType = 'townhouse';
    ...
    else result.objectType = 'house'; // материал не указан
  }
