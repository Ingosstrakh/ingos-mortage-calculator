...
function performCalculations(data) {
  ...
  try {
    ...
    if (variant2Result && variant2Result.output) {
      ...
      // ---> НАЧАЛО: Интеграция интерактивного конструктора
      try {
        const interactiveData = calculateInteractiveVariant2Data(data, bankConfig, insuranceAmount, totalWithoutDiscount);
        if (interactiveData) {
          console.log('Интерактивные данные для Варианта 2:', interactiveData);
          output += renderInteractiveVariant2UI(interactiveData);
        }
      } catch (e) {
        console.error('Ошибка при создании интерактивного Варианта 2:', e);
      }
      // <--- КОНЕЦ: Интеграция интерактивного конструктора
...
function renderInteractiveVariant2UI(interactiveData) {
    if (!interactiveData) return '';

    const { isSimplified, propertyPremium, lifePremium, titlePremium, mainRisk, additionalRisks, total } = interactiveData;

    let output = `<div id="interactive-variant-2" class="interactive-container" style="border: 1px solid #ccc; padding: 15px; margin-top: 15px; border-radius: 8px;">`;
    output += `<h4 style="margin-top: 0;">Конструктор Варианта 2</h4>`;

    // Базовые риски
    if (propertyPremium > 0) {
        output += `<div><span style="font-weight: bold;">Имущество:</span> ${propertyPremium.toFixed(2).replace('.', ',')} ₽</div>`;
    }
    if (lifePremium > 0) {
        output += `<div><span style="font-weight: bold;">Жизнь:</span> ${lifePremium.toFixed(2).replace('.', ',')} ₽</div>`;
    }
    if (titlePremium > 0) {
        output += `<div><span style="font-weight: bold;">Титул:</span> ${titlePremium.toFixed(2).replace('.', ',')} ₽</div>`;
    }

    output += `<hr style="margin: 10px 0;">`;

    // Основной дополнительный риск (нередактируемый)
    if (mainRisk) {
        output += `<div class="risk-row" style="padding: 5px 0;">
            <input type="checkbox" checked disabled data-premium="${mainRisk.premium}">
            <span style="font-weight: bold;">${mainRisk.name}</span> (${mainRisk.objects}) - ${mainRisk.premium.toFixed(2).replace('.', ',')} ₽
        </div>`;
    }

    // Редактируемые дополнительные риски
    additionalRisks.forEach((risk, index) => {
        output += `<div class="risk-row" style="padding: 5px 0;">
            <input type="checkbox" class="interactive-risk-checkbox" checked data-premium="${risk.premium}" data-sum="${risk.sum}" data-rate="${risk.premium / risk.sum}" id="risk-${index}">
            <label for="risk-${index}" style="margin-left: 5px;">${risk.name} (${risk.objects})</label>
            <input type="number" class="interactive-risk-sum" value="${risk.sum}" style="width: 100px; margin-left: 10px;"> ₽
            <span class="risk-premium" style="margin-left: 10px;">${risk.premium.toFixed(2).replace('.', ',')} ₽</span>
        </div>`;
    });
    ...
    // Итоговая сумма
    output += `<div style="font-weight: bold; font-size: 1.2em;">Итого: <span id="interactive-total">${total.toFixed(2).replace('.', ',')}</span> ₽</div>`;
...
/**
 * Новая функция для расчета данных для интерактивного Варианта 2.
 * Копирует логику calculateVariant2, но возвращает структурированный объект, а не HTML.
 */
function calculateInteractiveVariant2Data(data, bankConfig, insuranceAmount, variant1Total) {
  const isLifeOnly = data.risks.life && !data.risks.property && !data.risks.titul;
  if (isLifeOnly && !bankConfig.allow_discount_life) {
    return null;
  }
  ...
  if (!hasFullData) {
    ...
    return {
      isSimplified: true,
      propertyPremium: propertyPremiumV2,
      lifePremium: lifePremiumV2,
      titlePremium: 0,
      mainRisk: null,
      additionalRisks: [],
      total: totalV2,
      difference: variant1Total - totalV2
    };
  }

  let availableProducts = [];
  const isFlat = data.objectType === 'flat' || data.objectType === null;
  const isHouse = data.objectType === 'house_brick' || data.objectType === 'house_wood' || (data.objectType === 'house' && (data.material === 'brick' || data.material === 'wood'));
  ...
  if (data.risks.property) {
    ...
    if (propertyResult) {
      if (bankConfig.allow_discount_property) {
        const basePremium = propertyResult.totalWithoutDiscount;
        propertyPremiumV2 = Math.round(basePremium * 0.7 * 100) / 100;
        propertyPremiumV2 = Math.max(propertyPremiumV2, 600);
      } else {
        propertyPremiumV2 = propertyResult.totalWithoutDiscount || propertyResult.total;
      }
    }
  ...
  if (data.risks.life) {
    ...
    if (lifeResult) {
      ...
      if (bankConfig.allow_discount_life && !lifeResult.requiresMedicalExam && lifeResult.medicalUnderwritingFactor !== 1.25 && !hasAgeRestrictionForSberbank) {
        ...
        lifePremiumV2 = Math.max(totalWithDiscount, 600 * numBorrowers);
      } else {
        lifePremiumV2 = lifeResult.total || lifeResult.totalWithoutDiscount;
      }
    }
  ...
  for (const product of availableProducts) {
    if (product === 'lichnie_veschi') {
      const packs = window.LICHNIE_VESHCHI_PACKS;
      if (!packs) continue;
      const riskCombos = [{ key: 'povrezhd', label: 'ПДТЛ + Повреждения' }, { key: 'tipovye', label: 'ПДТЛ + Типовые' }, { key: 'all', label: 'все риски' }];
      for (const pack of packs) {
        for (const combo of riskCombos) {
          const premium = pack[combo.key];
          const totalV2 = propertyPremiumV2 + lifePremiumV2 + premium;
          productResults.push({ product: 'lichnie_veschi', productName: 'Личные вещи', riskName: combo.label, premium: premium, total: totalV2, packDetails: { pack, riskCombo: combo.key } });
        }
      }
    } else {
      const additionalRisk = calculateIFLAdditionalRisk(product, data, insuranceAmount);
      if (additionalRisk) {
        const totalV2 = propertyPremiumV2 + lifePremiumV2 + additionalRisk.premium;
        productResults.push({ product: product, productName: additionalRisk.productName, riskName: additionalRisk.riskName, premium: additionalRisk.premium, total: totalV2 });
      }
    }
  }
  ...
  const priorityProducts = productResults.filter(p => p.product === 'moyakvartira' || p.product === 'express' || p.product === 'lichnie_veschi');
  const otherProducts = productResults.filter(p => p.product !== 'moyakvartira' && p.product !== 'express' && p.product !== 'lichnie_veschi');
  priorityProducts.sort((a, b) => a.total - b.total);
  otherProducts.sort((a, b) => a.total - b.total);
  ...
  for (const product of priorityProducts) {
    ...
    if (difference >= 200) {
      if (difference <= 2200) {
        if (!bestProduct || productTotalWithTitle > (bestProduct.total + titlePremiumV2)) { bestProduct = product; bestDifference = difference; }
      } else {
        if (!bestProduct || Math.abs(difference - targetDifference) < Math.abs(bestDifference - targetDifference)) { bestProduct = product; bestDifference = difference; }
      }
    }
  }
  ...
  if (!bestProduct || bestDifference < 200) return null;

  let finalProduct = bestProduct;
  let finalAdditionalRisks = [];
  let currentTotal = bestProduct.total + titlePremiumV2;
  let currentDifference = variant1Total - currentTotal;
  const targetDifferenceLarge = 3000;
  ...
  if (currentDifference > targetDifferenceLarge) {
    if (bestProduct.product === 'moyakvartira') {
      ...
      if (moyaTariff) {
        if (insuranceAmount > 5000000) { baseFinishSum = 200000; } else { baseFinishSum = Math.min(500000, Math.max(200000, insuranceAmount * 0.08)); }
      }
      const additionalRisksResult = increaseMoyaKvartiraSumsForDifference(data, insuranceAmount, currentDifference, targetDifferenceLarge, baseFinishSum, variant1Total, propertyPremiumV2, lifePremiumV2, titlePremiumV2);
      if (additionalRisksResult && additionalRisksResult.risks.length > 0) {
        finalAdditionalRisks = additionalRisksResult.risks;
        currentTotal = propertyPremiumV2 + lifePremiumV2 + titlePremiumV2 + additionalRisksResult.totalPremium;
        finalProduct.premium = additionalRisksResult.totalPremium; 
      }
    } else if (bestProduct.product === 'bastion') {
      const bastionResult = increaseBastionSumsForDifference(data, insuranceAmount, currentDifference, targetDifferenceLarge, propertyPremiumV2, lifePremiumV2, titlePremiumV2, variant1Total);
      if (bastionResult) {
        finalProduct = bastionResult.finalProduct;
        finalAdditionalRisks = bastionResult.additionalRisks;
        currentTotal = bastionResult.currentTotal;
        const isFlat = data.objectType === 'flat' || data.objectType === null;
        const objectType = isFlat ? 'flat' : 'house';
        const bastionTariff = window.T_BASTION[objectType];
        if (bastionTariff) {
          ...
          const finishMax = Math.min(bastionTariff.finish.max, insuranceAmount);
          let finishSum;
          if (insuranceAmount < finishMin) { finishSum = Math.min(finishMin, finishMax); } else if (insuranceAmount > 5000000) { const maxReasonable = finishMin * 3; finishSum = Math.min(finishMax, Math.min(maxReasonable, Math.max(finishMin, insuranceAmount * 0.05))); } else { const maxReasonable = finishMin * 3; finishSum = Math.min(finishMax, Math.min(maxReasonable, Math.max(finishMin, insuranceAmount * 0.1))); }
          const finishPremium = Math.round(finishSum * bastionTariff.finish.rate * 100) / 100;
          finalAdditionalRisks.push({ name: 'Бастион', objects: `отделка и инженерное оборудование ${isFlat ? 'квартира' : 'дом'}`, sum: finishSum, premium: finishPremium, editable: true });
          currentTotal += finishPremium;
        }
      }
    } else if (bestProduct.product === 'express') {
  ...
  const riskDetails = getAdditionalRiskDetails(finalProduct.product, data, insuranceAmount, finalProduct.premium, finalAdditionalRisks, finalProduct.packDetails);
  const mainRisk = {
      product: finalProduct.product,
      name: finalProduct.productName,
      premium: finalProduct.premium,
      sum: riskDetails.sum, // Это может быть строка или число
      objects: riskDetails.objects,
      editable: false // Основной риск не редактируется
  };
  ...
  return {
    ...
    propertyPremium: propertyPremiumV2,
    lifePremium: lifePremiumV2,
    titlePremium: titlePremiumV2,
    mainRisk: mainRisk,
    additionalRisks: finalAdditionalRisks.map(risk => ({ ...risk, editable: true })),
    total: currentTotal,
    difference: variant1Total - currentTotal
  };
}
