...
function calculatePropertyInsurance(data, bankConfig, insuranceAmount) {
  ...
  // Применяем ручной тариф если задан
  let finalTariff = tariff;
  if (data.manualTariff) {
    finalTariff = data.manualTariff;
  }
  // Применяем коэффициент бани для кирпичного дома
  if (data.hasBanya && objectType === 'house_brick') {
    finalTariff = finalTariff * 1.23;
  }

  const premium = Math.round(insuranceAmount * (finalTariff / 100) * 100) / 100;
